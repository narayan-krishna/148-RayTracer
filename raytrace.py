...
def RT_trace_ray(scene, ray_orig, ray_dir, lights, depth=0):
    ...
    for light in lights:
        ...
        new_orig = hit_loc + hit_norm * .003 # replace the right hand side
        #
        ...
        #
        I_light = np.array((light_color/light_vec.dot(light_vec)))  # replace this line
        ...
        #
        # calculate half_vector (normalized vector of light dir and view dir)
        # calculate specular component, add that to the pixel color (k_specular * I_light)

        #specular component
        I_specular = np.array(specular_color) * I_light * hit_norm.dot((light_dir - ray_dir).normalized())**specular_hardness
        color += I_specular
